...
Local secrets: load from `.env` (Node) or `.streamlit/secrets.toml` (works for both types) or shell exports. Mirror the env-var names Keboola injects (`KBC_URL`, `KBC_TOKEN`, `WORKSPACE_ID`) so the same code paths run unchanged in both places. See [storage-access.md](storage-access.md) §Getting the env vars for local development for how to obtain each value (`KBC_URL`, `KBC_TOKEN`, `WORKSPACE_ID`).
