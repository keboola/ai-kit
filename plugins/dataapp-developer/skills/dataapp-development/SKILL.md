...
9. **For local-dev credentials: ASK the user, never scan.** When running an app locally (Path B/C, `streamlit run`, `kbagent` flows), the app needs Storage tokens, workspace IDs, and other credentials. **NEVER grep the filesystem, search shell history, or probe environment variables for tokens.** That's a security smell, error-prone, and wastes time. Instead, list the required env vars explicitly to the user and ask them to populate `.env` / `.env.local` (or `.streamlit/secrets.toml` for Streamlit) with values from the Keboola UI. Pause and wait for confirmation before attempting to run the app. The required values are documented in `references/storage-access.md` §Getting the env vars for local development.
