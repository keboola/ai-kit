...
7. **Pick one Keboola path per session.** Detect what's available at session start: any `mcp__*Keboola*` tool means an MCP is wired (could be project-local `.mcp.json`, user-level, or org-level — all surface the same way); `which kbagent && kbagent project list` succeeding means kbagent is wired. When more than one is present, **ask the user which to use** — they may point at different branches or projects, and mixing them produces silent inconsistencies. Locally with filesystem, kbagent + `streamlit run` iteration often beats MCP-only `modify_data_app` cycles for non-trivial apps. See `references/deployment-paths.md` §Pick one path per session.
