...
11. **Config does not travel with a git merge.** A draft and its prod app are separate Storage configurations. When you promote a draft, set the same `storage_access` on the prod app before deploying it — an app that reads Storage needs `storage_access=True` on *both*. Otherwise prod starts cleanly and fails at request time with `missing required env vars: WORKSPACE_ID`, which looks like a code regression and is not. See `references/python-js-prod-and-drafts.md` §6.
