...
- **BigQuery project** — identifier quoting, bucket→dataset mangling, read queries, the Query Service return shape (string cells, like Snowflake), and `INSERT` DML (via the Query Service: `rows_affected` populated, round-trip confirmed, statements share a session) are verified on a real BQ project (AJDA-2835, AJDA-2840). A `direct-grant` write to a real Storage table is also verified (2026-09-17: `INSERT`/`UPDATE`/`DELETE` through the Query Service, one row affected each) — it was broken until [DMD-1259](https://linear.app/keboola/issue/DMD-1259) / connection#8444 shipped on 2026-09-16.
