...
- **Prod-app-and-drafts flow** (`references/python-js-prod-and-drafts.md`) — written from the `keboola/mcp-server` tool contract and the AJDA-3263 measurement, not yet driven end to end from a fresh Apps-page app (rename prod → draft → push → `mode="dev"` preview → merge → prod deploy). Two claims to confirm on that run: whether a dev deployment follows the branch or stays pinned by commit locking, and that the preview authenticates on top of the draft's configured auth as the apps-proxy `kai-preview` design says (`keboola/ui` `apps/kai-agent/docs/data-apps-architecture.md`, AJDA-3237). Whatever that run shows, `"no-auth"` is not the answer for a preview — the MCP rejects it on drafts.
- **BigQuery project** — identifier quoting, bucket→dataset mangling, read queries, the Query Service return shape (string cells, like Snowflake), and `INSERT` DML (via the Query Service: `rows_affected` populated, round-trip confirmed, statements share a session) are verified on a real BQ project (AJDA-2835, AJDA-2840). A `direct-grant` write to a real Storage table from a *deployed* app is verified too (2026-09-17: a cell edit in the app's UI → `MERGE` via the Query Service → one row changed in Storage, row count unchanged).
