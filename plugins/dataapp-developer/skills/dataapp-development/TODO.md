...
- **Prod-app-and-drafts flow** (`references/python-js-prod-and-drafts.md`) — written from the `keboola/mcp-server` tool contract and the AJDA-3263 measurement, not yet driven end to end from a fresh Apps-page app (rename prod → draft → push → `mode="dev"` preview → merge → prod deploy). One claim to confirm on that run: whether a dev deployment follows the branch or stays pinned by commit locking. (Preview authentication is settled: the Kai preview iframe authenticates via an apps-proxy `kai-preview` cookie layered on top of the app's configured auth — see `keboola/ui` `apps/kai-agent/docs/data-apps-architecture.md` and AJDA-3237 — so `mode="dev"` never needs auth disabled.)
